...
import { PROMPTS, MARKET_ANALYSIS_PROMPT, CRYPTO_NEWS_ANALYSIS_PROMPT } from './prompts';
import {
  db,
  eq,
...
// New function to get CZ's personality configuration
async function getCZPersonalityConfig(key: string) {
  return db.query.personalityConfig.findFirst({
    where: eq(personalityConfig.key, `CZ_${key}`),
    columns: {
      value: true,
    },
  });
...
export async function generateReply({
  ...
  // If no system prompt provided, get CZ's personality configuration
  if (!systemPrompt) {
    const czConfig = await getCZPersonalityConfig('VOICE_CHARACTERISTICS');
    systemPrompt = czConfig?.value || await PROMPTS.TWITTER_REPLY_TEMPLATE();
  }
  ...
  let text = sanitizeLlmOutput(_text);

  // Add market analysis for relevant topics
    ...
    const question = await step.run('extract-question', async () => {
      ...
      const { text: extractedQuestion } = await generateText({
        ...
        messages: [
          {
            role: 'system',
            content: CRYPTO_NEWS_ANALYSIS_PROMPT,
          },
          {
            role: 'user',
            content: tweetText,
